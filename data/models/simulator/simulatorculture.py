...
class SimulatorCulture(models.Model):
    ...
    def create_from_airtable(airtable_json):
        ...
        if periodes_de_semis:
            try:
                sowing_period = SimulatorCulture.CulturesSowingPeriod[periodes_de_semis[0]].value
        ...
        if mois_semis:
            ...
            for mois in mois_semis:
                try:
                    sowing_months.append(SimulatorCulture.CulturesSowingMonth[mois].value)
                except KeyError as _:
                    pass

        return SimulatorCulture(
